...
const InclusaoLivros = () => {
    ...
    const salvar = async (campos) => {
        try {
            const res = await inAxios.post("livros", campos)
            setAviso(`Ok! Livro cadastrado com código ${res.data.id}`);
        } catch (error) {
    ...
    return (
        <div className="container">
            ...
            <div className={aviso.startsWith("Ok!") ? "alert alert-success" 
                : aviso.startsWith("Erro...") ? "alert alert-danger" : ""}>{aviso}</div>
        </div>
    );
}
