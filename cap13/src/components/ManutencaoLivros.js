...
const ManutencaoLivros = () => {
    ...
    const filtrarLista = async (campos) => {
        try {
            ...
            lista.data.length 
            ? setLivros(lista.data) 
            : alert(`Não há livros com a palavra chave pesquisa...`);
        } catch (error) {
            alert(`Error... Não foi possível obter os dados ${error}`);
        }
    };
    ...
    const alterar = async (id, titulo, index) => {
        ...
        try {
            ...
            alert(`Preço do livro alterado para: ${novoPreco}`)
        } catch (error) {
            alert(`Erro... Não foi possível alterar esse livro: ${error}`)
        }
    }
